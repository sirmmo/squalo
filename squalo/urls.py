...
urlpatterns = patterns('',
    ...
    url(r'^data/(?P<user>\w+)/(?P<db>\w+)$', 'squalo_core.views.database', name='db'),
    url(r'^data/(?P<user>\w+)/(?P<db>\w+)/(?P<model>\w+)$', 'squalo_core.views.model', name='model'),
    
    url(r'^api/(?P<user>\w+)$', 'squalo_core.views.user_api', name='user_api'),
    url(r'^api/(?P<user>\w+)/(?P<db>\w+)$', 'squalo_core.views.database_api', name='db_api'),
    url(r'^api/(?P<user>\w+)/(?P<db>\w+)/(?P<model>\w+)$', 'squalo_core.views.query', name='model_api'),
